...
\begin{algorithm}[H]
	...
	\label{des:a:ql}
\end{algorithm}
\footnotetext{The pseudocode has been taken from~\textcite{rlIntro}}
...
\begin{figure}[th]
    ...
    \node[] at (axis cs: -5,-3) {Region 1};
    \node[] at (axis cs: -3,3) {Region 2};
    \node[] at (axis cs: 3,3) {Region 3};
    \node[] at (axis cs: 5,-3) {Region 4};
    \end{axis}
    \end{tikzpicture}
    \centering
...
The following describes the method that optimized initializer applies to four regions defined in previous section. Table~\ref{des:tab:rules} summarizes each region and describes preferred action in each region.
\begin{description}[leftmargin=0pt,nolistsep]
...
\begin{figure}[H]
    ...
    \resizebox{0.8\textwidth}{!}{
        \begin{tikzpicture}[show background grid]
            ...
            \end{interface}
            \begin{class}[text width=6cm]{GreedyPolicy}{0,-2.5}
            \implement{Policy}
            \operation{+ nextAction(currentState: State)}
            \end{class}
            \begin{class}[text width=6cm]{OneMinusEpsilonPolicy}{8,-2.5}
            \implement{Policy}
            \attribute{- underlyingPolicy: Policy}
            \operation{+ nextAction(currentState: State)}
            \end{class}
            \begin{class}[text width=6cm]{DecreasingOneMinusEpsilonPolicy}{8,-5}
...
\begin{figure}[H]
    ...
    \resizebox{0.8\textwidth}{!}{
        ...
        \end{interface}
        \begin{class}[text width=9cm]{StaticExecutorStrategy}{0,-3}
        ...
        \end{class}
        \begin{class}[text width=9cm]{LinearExecutorStrategy}{4,-8}
        \implement{ExecutorStrategy}
        \operation{+ howManyExecutorsToAdd(totalExecutors: Integer)}
        \operation{+ howManyExecutorsToRemove(totalExecutors: Integer)}
        \end{class}
        \begin{class}[text width=9cm]{QueueAwareExecutorStrategy}{7,-5.5}
        \implement{ExecutorStrategy}
        \operation{+ howManyExecutorsToAdd(totalExecutors: Integer)}
        \operation{+ howManyExecutorsToRemove(totalExecutors: Integer)}
        \end{class}
...
\begin{table*}[h]
    \begin{tabular}{ll}
        ...
        Linear & Add or remove with a fixed coefficient for same consecutive actions.\\
