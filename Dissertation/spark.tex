...
\begin{figure}[hb]
    \centering
    \includegraphics[clip,trim=3cm 23cm 3.5cm 2.1cm,scale=0.98]{dstream-high.pdf}
...
\begin{figure}[h]
    ...
    \caption[Streaming Word Count using D-Stream API]{Streaming Word Count using D-Stream API}
    \label{fig:sp:dstream-api}
\end{figure}
\footnotetext{The code and figure has been partially taken from~\cite{Zaharia:2013}}

\clearpage
\subsection{D-Stream Fault-Tolerance}
...
Naturally, in a streaming applications, live stream of records are coming from some data sources. For example, in log processing application it may originate from web servers or in live monitoring applications it comes from sensors. Now the questions is what happens if the first transformation that reads data from live stream fails? Are the consumed messages lost? or it can be replayed.
\section{Conclusion}
\label{sp:conc}
