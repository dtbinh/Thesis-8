...
import com.sap.rm.fugu.FuguResourceManager
import com.sap.rm.fugu.model.BatchTier
import com.sap.rm.fugu.simulation.BatchSimulator
...
class ResourceOptimizer(manager: FuguResourceManager) {
  def calculateExecutors(batches: Seq[BatchTier]): OptimizerResult = {
    val lowerBound = max(manager.config.MinimumExecutors, 1)
    val upperBound = max(manager.config.MaximumExecutors, lowerBound)
    val parameters = new ParameterSpace(manager, batches)
    val executorNo = search(lowerBound, upperBound, parameters)
