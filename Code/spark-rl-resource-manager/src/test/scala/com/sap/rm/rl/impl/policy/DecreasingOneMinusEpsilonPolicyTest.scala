...
class DecreasingOneMinusEpsilonPolicyTest extends FunSuite {

  val sparkConf: SparkConf = createSparkConf()

  test("testNextActionFrom") {
    sparkConf.set(InitializationModeKey, "zero")
    sparkConf.set(EpsilonKey, "0.31")
    sparkConf.set(EpsilonStepKey, "0.1")
    ...
    val policy = DecreasingOneMinusEpsilonPolicy(config, GreedyPolicy(config), generator)

    assert(policy.nextActionFrom(stateSpace, State(MinimumExecutors, 7, loadIsIncreasing = true), NoAction, State(MinimumExecutors, 8, loadIsIncreasing = true)) == NoAction)
